<script lang="ts">
	...
	interface Props {
		...
		projects?: Project[];
		initialId?: string;
		projectView?: 'list' | 'grid';
	}

	const { projects = [], initialId, winId, projectView: projectViewProp }: Props = $props();
	const lang = $derived(systemStore.lang);
	const msg = $derived(getMessages(lang));

	let filter = $state<'all' | 'personal' | 'clients'>('all');
	let localView = $state<'list' | 'grid'>('list');
	const effectiveView = $derived(projectViewProp ?? localView);
	...
	function setView(v: 'list' | 'grid') {
		localView = v;
		if (winId) wmStore.updateProps(winId, { projectView: v });
	}
	...
	$effect(() => {
		if (systemStore.deviceClass === 'mobile' && localView === 'list') {
			localView = 'grid';
		}
	});
	...
	const detailProject = $derived(projects.find((p) => p._id === detailId) ?? null);
</script>

<div class="module" style="flex-direction:column">
	<div class="toolbar" style="position:relative">
		{#if detailId !== null}
			<button class="tb-btn" onclick={() => (detailId = null)}><ArrowLeft size={13} /> {lang === 'de' ? 'Zurück' : 'Back'}</button>
			<div class="sep"></div>
			<span class="tb-label">{detailProject ? pickLocale(lang, detailProject.title) : ''}</span>
		{:else}
	...
	<div style="display:flex;flex:1;overflow:hidden">
		{#if detailId !== null && detailProject}
			<div class="module-main">
				<div style="padding:20px 28px 32px">
					<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.14em;margin-bottom:8px">
						{detailProject.type === 'personal' ? msg.personal() : msg.clients()} · {detailProject.year}
						{#if detailProject.openSource}<span style="margin-left:8px;color:var(--accent-2)">open source</span>{/if}
					</div>
					<h1 class="h-display" style="font-size:26px;margin-bottom:12px;font-family:var(--font-display)">
						{pickLocale(lang, detailProject.title)}
					</h1>
					<p style="font-size:14px;color:var(--text-1);max-width:620px;line-height:1.65;margin-bottom:18px">
						{pickLocale(lang, detailProject.summary)}
					</p>

					<div class="ph-image cover-image project-cover" style="height:260px;margin-bottom:24px">
						{#if detailProject.cover}
							<img
								src={urlFor(detailProject.cover).width(1200).height(520).fit('crop').auto('format').url()}
								alt={pickLocale(lang, detailProject.title)}
								class="cover-img"
							/>
							<div class="cover-dim"></div>
						{/if}
						<div class="ph-cap">{detailProject._id} · cover</div>
						<div class="ph-tag">{detailProject.year}</div>
					</div>

					<div style="display:grid;grid-template-columns:120px 1fr;row-gap:8px;column-gap:18px;font-size:12.5px;max-width:720px">
						{#if detailProject.type === 'client' && detailProject.client}
							<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.12em;padding-top:2px">{msg.client()}</div>
							<div style="color:var(--text-1)">{detailProject.client}</div>
						{:else if detailProject.type === 'personal' && detailProject.context}
							<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.12em;padding-top:2px">{msg.context()}</div>
							<div style="color:var(--text-1)">{pickLocale(lang, detailProject.context)}</div>
						{/if}
						<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.12em;padding-top:2px">{msg.role()}</div>
						<div style="color:var(--text-1)">{pickLocale(lang, detailProject.role)}</div>
						{#if detailProject.stack && detailProject.stack.length > 0}
							<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.12em;padding-top:2px">{msg.stack()}</div>
							<div class="mono dim">{detailProject.stack.join(' · ')}</div>
						{/if}
						{#if detailProject.tags && detailProject.tags.length > 0}
							<div class="dim mono" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.12em;padding-top:2px">Tags</div>
							<div class="mono dim">{detailProject.tags.map((x) => '#' + x).join('  ')}</div>
						{/if}
						{#if detailProject.links && detailProject.links.length > 0}
							...
							<div style="display:flex;flex-wrap:wrap;gap:6px 14px">
								{#each detailProject.links as link (link.url)}
									<a
										href={link.url}
										target="_blank"
				...
				<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:14px">
					{#each filtered as p (p._id)}
						<button
							...
							onclick={() => { selectedId = p._id; detailId = p._id; }}
						>
							<div class="ph-image cover-image project-cover" style="height:130px">
								{#if p.cover}
									<img
